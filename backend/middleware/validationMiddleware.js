...
const schemas = {
  ...
  addResourceSchema: Joi.object({
    ...
    url: Joi.string().uri().allow("").messages({
      // URL is optional, only if type is 'link'
  ...
  addYoutubeLinkSchema: Joi.object({
    ...
    url: Joi.string()
      ...
      .messages({
        ...
        "string.uri": "URL must be a valid URI",
        "string.pattern.base":
          "Please enter a valid YouTube URL (e.g., youtube.com/watch?v=...)",
  ...
  addFeedbackSchema: Joi.object({
    ...
    category: Joi.string()
      ...
      .messages({
        "any.only":
          "Category must be one of: general, technical, suggestion, complaint, feature-request, other",
  ...
  updateFeedbackStatusSchema: Joi.object({
    status: Joi.string()
      ...
      .messages({
        "any.only":
          "Status must be one of: pending, reviewed, resolved, closed",
        "any.required": "Status is required",
      }),
    adminResponse: Joi.string().trim().allow("").max(1000).messages({
      "string.max":
        "Admin response should have a maximum length of {#limit} characters",
    }),
  }),
  ...
  addCalendarEventSchema: Joi.object({
    ...
    description: Joi.string().trim().min(10).required().messages({
      ...
      "string.min":
        "Event description should have a minimum length of {#limit}",
    ...
    eventType: Joi.string()
      ...
      .messages({
        "any.only":
          "Event type must be one of: holiday, exam, assignment, meeting, seminar, workshop, other",
      }),
  }),
        ...
        .items(
          Joi.object({
            ...
            timeSlot: Joi.string()
              ...
              .messages({
                "string.pattern.base":
                  "Time slot must be in format HH:MM-HH:MM (e.g., 09:00-10:00)",
              }),
            room: Joi.string().trim().allow(""),
            type: Joi.string()
              .valid("lecture", "lab", "tutorial", "break")
              .default("lecture"),
          }),
        )
        .default([]),
  ...
  return (req, res, next) => {
    ...
    if (!schema) {
      ...
      req.session.messages = [
        "Internal server error: Validation schema missing.",
      ];
    ...
    if (source === "body") {
      ...
      console.error(
        `Invalid source for validation: ${source}. Must be 'body' or 'query'.`,
      );
      req.session.messages = [
        "Internal server error: Invalid validation source.",
      ];
      return res.redirect(getSafeRedirectUrl(req, schemaName));
    }
