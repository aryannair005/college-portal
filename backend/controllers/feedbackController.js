...
exports.postAddFeedback = async (req, res) => {
  try {
    const { title, message, category, priority, rating, isAnonymous } =
      req.body;
    ...
    const feedback = new Feedback({
      ...
      submittedByName:
        isAnonymous === "true" ? "Anonymous" : req.session.username,
    });
...
exports.getFeedbackDetail = async (req, res) => {
  try {
    ...
    if (
      ...
      feedback.submittedBy._id.toString() !== req.session.userId
    ) {
...
exports.exportFeedback = async (req, res) => {
  try {
    ...
    // Simple CSV export
    let csv =
      "Title,Message,Category,Priority,Status,Submitted By,Submitted At,Rating\n";
