  ...
  try {
    ...
    if (type === "pdf" && !req.file) {
      req.session.messages = [
        "Please upload a PDF file for PDF resource type.",
      ];
      return res.redirect("/admin/add-resource");
    }
    if (type === "link" && (!url || !url.startsWith("http"))) {
      // Basic check, Joi schema has uri validation
      req.session.messages = [
        "Please provide a valid URL for link resource type.",
      ];
      return res.redirect("/admin/add-resource");
    }
...
exports.deleteResource = async (req, res) => {
  try {
    ...
    req.session.messages = [
      "Resource and all associated YouTube links deleted successfully",
    ];
...
exports.addYoutubeLink = async (req, res) => {
  ...
  try {
    ...
    if (existingLink) {
      req.session.messages = [
        "This YouTube URL is already added to this resource",
      ];
...
exports.bulkAddYoutubeLinks = async (req, res) => {
  ...
  try {
    ...
    for (let link of links) {
      ...
      const exists = resource.youtubeLinks.find(
...
exports.postAddSyllabus = async (req, res) => {
  // Joi validation handled by middleware
  try {
    const { title, course, semester, subject, description, type, url } =
      req.body;

    if (type === "pdf" && !req.file) {
      req.session.messages = [
        "Please upload a PDF file for PDF syllabus type.",
      ];
      return res.redirect("/admin/add-syllabus");
    }
    if (type === "link" && (!url || !url.startsWith("http"))) {
      req.session.messages = [
        "Please provide a valid URL for link syllabus type.",
      ];
      return res.redirect("/admin/add-syllabus");
    }
