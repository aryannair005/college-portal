...
router.post(
  "/add",
  ensureStudent,
  upload.single("doubtImage"),
  validate("addDoubtSchema"),
  doubtController.postAddDoubt,
); // Add validation
router.get("/:id", ensureStudent, doubtController.getSingleDoubt);
router.post(
  "/:id/reply",
  ensureStudent,
  validate("replyToDoubtSchema"),
  doubtController.postReplyToDoubt,
); // Add validation
router.post(
  "/:id/solved",
  ensureStudent,
  doubtController.postMarkDoubtAsSolved,
);
